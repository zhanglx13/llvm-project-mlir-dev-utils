...
printUsage()
{
    ...
    echo "      use -o <lowestIRFilename> to secify the filename"
    echo "      If not specifies, wrapper_lowest.mlir will be used"
    echo "      Use -w to print only the wrapper function"
...
## $2: 1: extract the wrapper function
...
print_lowestIR()
{
    ...
    if [[ $2 -eq 1 ]]; then
        ...
        sed -i '/llvm.func @miopen/,/}/!d;/}/q' lowest.mlir > wrapper_lowest.mlir
    fi
}
...
cpuPipeline=0
wrapper_func=0
while getopts "hrlo:m:vc:gi:dw" opt; do
    case "$opt" in
        h)
            printUsage
            exit 0
...
if [[ $driverPipeline -eq 1 ]]; then
    ...
    if [[ $lowestIR -eq 1 ]]; then
        print_lowestIR driver_output.mlir ${wrapper_func}
    fi
